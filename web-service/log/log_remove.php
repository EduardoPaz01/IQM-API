...
try {
    if ($id) {
        $stmt = $con->prepare('DELETE FROM `log` WHERE idlog = ?');
    ...
    if ($ids) {
        ...
        $sql = "DELETE FROM `log` WHERE idlog IN ($placeholders)";
    ...
    echo json_encode(['success' => false, 'message' => 'Informe idlog (int) ou ids (array) para exclusão.']);
    $con->close();
} catch (Exception $e) {
    echo json_encode(['success' => false, 'message' => $e->getMessage()]);
    if (isset($stmt) && $stmt) $stmt->close(); $con->close();
}
